import 'dart:ui';

class AppColors {
  static const Color bgColor = Color(0xffF3EFEF);
  static const Color primaryColor = Color(0xffE0001B);
  static const Color primaryShade100 = Color(0xffF5B0B8);
  static const Color appGreyColor = Color(0xff848484);
  static const Color appBarTitleColor = Color(0xff333333);
  static const Color borderColor = Color(0xffF18A96);
  static const Color secondaryColor = Color(0xffE20070);
  static const Color secondaryColorShade300 = Color(0xffEC549F);
  static const Color secondaryColorShade100 = Color(0xffFCE6F1);
  static const Color darkColor = Color(0xff000000);
  static const Color successColor = Color(0xff00A213);

  static List<Color> splashLinearColors = [
    Color(0xffE1559B).withOpacity(0.2),
    Color(0xffFFFFFF),
  ];
}
