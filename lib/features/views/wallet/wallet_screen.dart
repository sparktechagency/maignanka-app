              ...
              builder: (controller) {
                if (controller.selectedValueType == 'gifts') {
                  ...
                  if (controller.giftHistory.isEmpty) {
                    return Center(
                      child: CustomText(text: 'No gift history found.'),
                    );
                  }
                  ...
                  if (controller.topUpHistory.isEmpty) {
                    return Center(
                      child: CustomText(text: 'No top-up history found.'),
                    );
                  }
                  return ListView.builder(
                    controller: _scrollController,
    ...
    return ListTile(
      ...
      title: CustomText(
        color:
            data.type == 'withdraw'
                ? AppColors.primaryColor
                : (data.isAccepted == true && data.type == 'withdraw')
                ? Colors.amber
                : AppColors.successColor,
        textAlign: TextAlign.start,
        text:
                  ...
                  children: [
                    CustomTextField(
                      ...
                      onChanged:
                          (value) =>
                              isTopUp
                                  ? controller.calculateTopUp(value)
                                  : controller.calculateWithdraw(value),
                    ),
                    SizedBox(height: 10),
                    Row(
                      children: [
